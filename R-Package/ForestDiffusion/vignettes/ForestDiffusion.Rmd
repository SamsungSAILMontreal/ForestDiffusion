...
Let's use the Iris dataset as an example. Since it performs better, we will use the flow method to generate fake samples. Note that the dataset can contain missing values since XGBoost can handle NAs, yet the generated data will never have missing values (isn't it great?). For the sake of the vignette, we use $n_t=10$, and $n_{cores}=1$, but $n_t \geq 50$ is recommended and using more cores (no limit: set $n_{cores}$ to NULL or use high values such as $n_{cores}=8$) would make it train faster.
...
forest_model = ForestDiffusion(X=Xy, n_cores=1, n_t=10, duplicate_K=100, flow=TRUE, seed=666)
...
# forest_model = ForestDiffusion(X=X, n_cores=1, label_y=y, name_y='Species', n_t=10, duplicate_K=100, flow=TRUE, seed=666)
# Xy_fake = ForestDiffusion.generate(forest_model, batch_size=NROW(X), seed=666)
# plot(Xy_fake)
...
Below, we show how to impute missing data using ForestDiffusion. For the sake of the vignette, we use $n_t=10$, and $n_{cores}=1$, but $n_t \geq 50$ is recommended and using more cores (no limit: set $n_{cores}$ to NULL or use high values such as $n_{cores}=8$) would make it train faster.

```{r}
library(mice)

# Must train a VP diffusion model (instead of a Flow model) to be able to impute data
forest_model_vp = ForestDiffusion(X=Xy, n_cores=1, n_t=10, duplicate_K=100, flow=FALSE, seed=666)
...
imp1 = Xy_imp[[1]] # first imputed dataset
```
